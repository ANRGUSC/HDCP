...
static void recv_from_broadcast(struct broadcast_conn *c, const rimeaddr_t *from)
{
	...
	rimeaddr_t 		addr_broadcast;  // Null address
	...
	/*if(rimeaddr_cmp(&addr_final_destination, &rimeaddr_node_addr)) {
	if(bc->cb->recv != NULL) {
	bc->cb->recv(bc,hdr);
	} else {
...
{
	//PRINTF("sent_from_broadcast called %d,\n",transmissions);	
...
//	if(OWNDEBUG1)
	...
	if((hdcp_conn->current_packet->hdr).origin_seq_no==packetbuf_attr(PACKETBUF_ATTR_PACKET_ID)) //check 1. TODO: Add source check
	{
		...
		if(i != NULL) 
		{
			...
			// routingtable_update_link_success(&hdcp_conn->routing_table, &hdcp_conn->active_neighbor, transmissions);
...
static void send_restrans(void *ptr)
{
	...
	if(q != NULL) 
	{
		...
		packetbuf_set_attr(PACKETBUF_ATTR_RELIABLE, 1);
  		...
  		max_mac_rexmits = c->max_rexmits - c->transmissions > MAX_MAC_REXMITS? MAX_MAC_REXMITS : c->max_rexmits - c->transmissions;
  		packetbuf_set_attr(PACKETBUF_ATTR_MAX_MAC_TRANSMISSIONS, max_mac_rexmits);
  		packetbuf_set_attr(PACKETBUF_ATTR_NUM_REXMIT,packetbuf_attr(PACKETBUF_ATTR_NUM_REXMIT)+c->transmissions);
